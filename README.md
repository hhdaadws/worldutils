...
- A* 寻路采用阻尼式平滑转向和拐点预瞄；直线段疾跑，明显转弯、接近交互目标和跳台阶时自动降为步行
...
- 每个 zone 只有在“成熟数 ≥ 当前已种作物数的一半”时才加入收获，空耕地不计入分母
- 所有达到 50% 的 zone 加入待处理队列，但严格逐个 zone 收获，不会跨区选择下一株
- 进入 zone 时从离玩家最近的角开始，沿区域较长边按蛇形顺序连续种植/收获；蛇形完成后刷新扫描，并用原来的最近目标策略做一次遗漏兜底
- 当前 zone 会先尽量补种，再完成成熟作物收获；种子箱为空时仅该作物进入5分钟冷却并左键回退，不阻塞当前区完成
