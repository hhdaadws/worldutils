package com.worldutils;

import com.worldutils.bot.Bot;
import com.worldutils.bot.Recorder;
import com.worldutils.config.ModConfig;
import com.worldutils.farm.FarmBot;
import com.worldutils.farm.FarmCommands;
import com.worldutils.farm.FarmConfig;
...
public class WorldUtilsClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ModConfig.load();
        FarmConfig.load();
        com.worldutils.stash.StashConfig.load();
        com.worldutils.license.LicenseManager.load();

        toggleKey = KeyBindingHelper.registerKeyBinding(new KeyBinding(
                "key.worldutils.toggle",
                InputUtil.Type.KEYSYM,
                GLFW.GLFW_KEY_J,
                "category.worldutils"));

        farmToggleKey = KeyBindingHelper.registerKeyBinding(new KeyBinding(
                "key.worldutils.farmToggle",
                InputUtil.Type.KEYSYM,
                GLFW.GLFW_KEY_K,
                "category.worldutils"));
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            com.worldutils.stash.StashBot.INSTANCE.tick(client);
        });

        ClientPlayConnectionEvents.DISCONNECT.register((handler, client) -> {
            Bot.INSTANCE.hardStop();
            FarmBot.INSTANCE.hardStop();
            com.worldutils.stash.StashBot.INSTANCE.hardStop();
            Recorder.cancel();
        });
    ...
    private void registerCommands() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            ...
            com.worldutils.stash.StashCommands.register(dispatcher);
            dispatcher.register(ClientCommandManager.literal("miner")
                        ...
                        .executes(ctx -> {
                            feedback(ctx.getSource(), """
                                    ...
                                    §e/miner license <码>§7 - 输入验证码激活授权 (无参数查看剩余时间)
                                    §e/miner login <账号> <密码>§7 - 账号密码登录授权 (有效期到账号到期日)
