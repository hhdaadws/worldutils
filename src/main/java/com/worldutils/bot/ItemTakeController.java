...
public class ItemTakeController {
    ...
    private final Consumer<String> msgSink; // 首次记录物品 id 时的提示出口（挖矿/存物前缀不同）
    ...
    public ItemTakeController(BlockPos chestPos, Supplier<String> idGet, Consumer<String> idSet,
        ...
        this(chestPos, idGet, idSet, label, maxStacks, Bot::msg);
    }

    public ItemTakeController(BlockPos chestPos, Supplier<String> idGet, Consumer<String> idSet,
                              String label, int maxStacks, Consumer<String> msgSink) {
    ...
    public Result tick(MinecraftClient mc, ClientPlayerEntity player) {
        ...
        for (Slot slot : handler.slots) {
            ...
            if (idGet.get() == null) {
                idSet.accept(id);
                msgSink.accept("§7已记录" + label + "物品: §e" + id);
