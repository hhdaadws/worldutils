...
public final class FarmItems {
    ...
    /**
     * 清空主手（切到空快捷栏格；快捷栏全满则把手上物品 SWAP 到主背包空格）。
     * 主手已空返回 true；发起了 SWAP 需等 1 tick 同步返回 false；背包全满也返回 false。
     * 用于"空手右键收获"——手里有种子会被插件复种，必须先空手。
     */
    public static boolean selectEmptyHand(MinecraftClient mc, ClientPlayerEntity player) {
