...
public class Bot {
    ...
    public void start() {
        ...
        if (com.autominer.farm.FarmBot.INSTANCE.isRunning()) {
    ...
    public void tick(MinecraftClient mc) {
        ...
        // 移动类状态中玩家自己打开了界面（聊天/背包）→ 暂停
        boolean movementState = state == State.MINING || state == State.PATH_TO_CHEST
                || state == State.PATH_TO_POTION || state == State.PATH_TO_FOOD
                || state == State.PATH_TO_PICK;
    ...
    private void retryTp(State tpState) {
        ...
        if (tpRetries >= MAX_TP_RETRIES) {
            stop("§c传送反复失败（已试 " + MAX_TP_RETRIES + " 次），停止。请检查传送设置");
            return;
        }
    ...
    private void tickMining(MinecraftClient mc, ClientPlayerEntity player, ModConfig cfg) {
        ...
        if (r == MiningController.Result.HAZARD_STOP) {
            stop("§c" + mining.getHazardMessage());
        }
    }
    ...
    private void tickDeposit(MinecraftClient mc, ClientPlayerEntity player, ModConfig cfg) {
        ...
        if (!nextChest(cfg)) {
            if (itemsRemain) {
                stop("§c所有绑定箱子都满了，物品存不完，已停止");
            } else {
                stop("§c所有箱子里都没有耐久充足的镐子，已停止");
            }
        }
    }
    ...
    private void tickPathToPick(MinecraftClient mc, ClientPlayerEntity player, ModConfig cfg) {
        ...
        if (pathExecutor == null) {
            List<BlockPos> path = Pathfinder.findToChest(mc.world, player.getBlockPos(), chest, 20000);
            if (path == null) {
                stop("§c走不到镐子箱 " + cfg.pickChest + "，已停止");
                return;
            }
            pathExecutor = new PathExecutor(path);
            return;
    ...
    public static void msg(String s) {
        MinecraftClient mc = MinecraftClient.getInstance();
        if (mc.player != null) {
            mc.player.sendMessage(Text.literal("§6[AutoMiner]§r " + s), false);
        }
    }
